...
int ic_status(struct i2c_client *client)
{
...
int __hello_packet_handler(struct i2c_client *client, int chip_type)
{
	int ret = 0;
	struct elan_ts_data *ts = i2c_get_clientdata(client);
	
	dev_info(&client->dev, "[elan] chip_type =%d\n", chip_type);
	if (HID_TYPE_PROTOCOL == chip_type) {
		ret = ic_status(client);
...
{
	const int pen_osr = 256; /*Ntring=256, Warcon: 260*/
	...
	fw_info->rx = rbuf[7];
	/*finger resolution*/
	fw_info->finger_xres = (rbuf[6] - 1) * fw_info->finger_osr;
	fw_info->finger_yres = (rbuf[7] - 1) * fw_info->finger_osr;

	/*pen resolution*/
	fw_info->pen_xres = (rbuf[6] - 1) * pen_osr;
...
int __fw_packet_handler(struct i2c_client *client)
{
	struct elan_ts_data *ts = i2c_get_clientdata(client);
	int ret = 0;
...
static int elan_ts_recv_data(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	if ( rc < 0) {
		dev_err(&ts->client->dev, "[elan] recv report data error!!\n");
		return -1;
	}
	...
	if (ts->chip_type == HID_TYPE_PROTOCOL) {
		if (rbuf[2] == HID_FID) {
			...
			report->stylus.pbutton_value		= rbuf[13];
...
static void elants_slot_report(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	if (report->tool_type == ELAN_FINGER) {
		...
		if(stylus.inrange_status) {
			 ...
			 dev_info(&ts->client->dev, "[elan] pen X:Y:P ====%d:%d:%d\n",x,y,p);
		} else {
			input_mt_slot(ts->pen_idev, 0);
...
static int elan_request_pen_input_dev(struct elan_ts_data *ts)
{
	...
	if (ts->report_type == PROTOCOL_TYPE_B) {
		...
		input_set_abs_params(ts->pen_idev, ABS_MT_POSITION_X, 0,  800, 0, 0);
		input_set_abs_params(ts->pen_idev, ABS_MT_POSITION_Y, 0,  1200, 0, 0);
		...
		input_set_abs_params(ts->pen_idev, ABS_X, 0,  800, 0, 0);
		input_set_abs_params(ts->pen_idev, ABS_Y, 0,  1200, 0, 0);
...
static int elan_request_finger_input_dev(struct elan_ts_data *ts)
{
	...
	input_set_abs_params(ts->finger_idev, ABS_MT_POSITION_X, 0, 800, 0, 0);
	input_set_abs_params(ts->finger_idev, ABS_MT_POSITION_Y, 0, 1200, 0, 0);
...
#ifdef IAP_PORTION
	dev_info(&ts->client->dev, "[elan]Start IAP Flow!!!\n");
...
static int elan_ts_setup(struct elan_ts_data *ts)
{
	...
	err = __hello_packet_handler(ts->client, ts->chip_type);
	if ( err < 0 ) {
		dev_err(&ts->client->dev,
...
static void elan_touch_node_init(struct elan_ts_data *ts)
{
    ...
    ts->p = proc_create("elan-iap", 0664, NULL, &elan_touch_fops);
    if (ts->p == NULL)
...
*/
	...
	hw_info->intr_gpio = of_get_named_gpio_flags(dev.of_node,
	...
	hw_info->rst_gpio = of_get_named_gpio_flags(dev.of_node,
	...
	ret = of_property_read_u32(dev.of_node, "chip_type", &data);
	if (ret == 0) {
		ts->chip_type = data;
		dev_info("[elan]:chip protocol_type=%s", ts->chip_type == 1 ? "HID IIC":"NORMAL IIC" );
	} else {
		ret = -EINVAL;
	...
	/*get report protocol */
	ret = of_property_read_u32(dev.of_node, "report_type", &data);
	if (ret == 0) {
		ts->report_type = data;
		dev_info("[elan]:report protocol_type=%s", ts->report_type == 1?"B protocol":"A protocol");
	} else {
	...
	/*get lcm coordinate*/
	ret = of_property_read_u32_array(dev.of_node, "lcm_resolution", lcm_coordinate,sizeof(lcm_coordinate));
	if (ret == 0) {
		ts->hw_info.screen_x = lcm_coordinate[0];
		ts->hw_info.screen_y = lcm_coordinate[1];
		dev_info("[elan]:LCM RESOLUTION X:Y=%d:%d,", ts->hw_info.screen_x, ts->hw_info.screen_y);
	} else {
		ret = -EINVAL;//hw_info->rst_gpio;
...
{
	struct input_dev *fidev = private_ts->finger_idev;
	struct input_dev *pidev = private_ts->pen_idev;
	int i = 0;
...
try_set_power:  // if system would not power off, must do this and check
		...
		dev_info(&client->dev, "[elsn] %s Nothing Done!!\n",__func__);
	}
