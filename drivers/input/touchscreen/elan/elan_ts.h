...
//extern function
extern void check_update_flage(struct elan_ts_data *ts);
extern int __fw_packet_handler(struct i2c_client *client);
extern  int __hello_packet_handler(struct i2c_client *client, int chip_type);
extern void elan_ts_hw_reset(struct ts_chip_hw_info *hw_info);
extern void elan_switch_irq(int on);
extern int elan_ts_calibrate(struct i2c_client *client);
extern int FW_Update(struct i2c_client *client);
extern int ic_status(struct i2c_client *client);
...
extern int get_vendor_fw(struct elan_ts_data *ts, int type);
extern int tp_module_test(struct elan_ts_data *ts);
