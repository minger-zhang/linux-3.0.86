...
{
	{0x2ae2, "T100_0x5610", active_pen_fw, sizeof(active_pen_fw), 0x271F},
...
static int hid_fw_upgrade_init(struct elan_ts_data *ts)
{
	...
	err = ic_status(ts->client);
	if (err != UNKNOW_TYPE) {
		dev_info(&ts->client->dev, "[elan] %s bcl = 0x%02x\n", __func__,ts->fw_info.fw_bcl);
	...
	dev_info(&ts->client->dev, "[elan] %s get ic status = %d\n", __func__, err);
...
update_retry:
	...
	msleep(50);
	dev_info(&ts->client->dev, "[elan]%s fw size = %d\n", __func__,wFwSize);
	while(wFwSize) {
	...
	if (err) {
		dev_info(&ts->client->dev,"[elan] fw upgrade finshed failed!!!\n");	
		return -1;
	}

	dev_info(&ts->client->dev,"[elan] fw upgrade success!!!\n");	
	return 0;
...
int FW_Update(struct i2c_client *client)
{
	int err = 0;
	struct elan_ts_data *ts = i2c_get_clientdata(client);
...
int elan_ts_hid_calibrate(struct i2c_client *client)
{
...
int elan_ts_normal_calibrate(struct i2c_client *client)
{
	struct elan_ts_data *ts = i2c_get_clientdata(client);
...
static int get_driver_fw(struct elan_ts_data *ts)
{
	...
	dev_info(&ts->client->dev,"[elan] ID is error, not support!!\n");
	return -1;
}	


int get_vendor_fw(struct elan_ts_data *ts, int type)
{
	int err = 0;
	struct elan_update_fw_info *update_info = &ts->update_info;
...
void check_update_flage(struct elan_ts_data *ts)
{
	...
	** support three methods to get fwdata.
	...
	err = get_vendor_fw(ts,ts->fw_store_type);
	if ( err ) {
	...
	*/
	New_FW_ID = update_info->FwData[0xD5EB] << 8 | update_info->FwData[0xD5EA];
	New_FW_VERSION = update_info->FwData[0xD1DF] << 8 | update_info->FwData[0xD1DE];
...
fw_upgrade:
	...
	/*get fw msg*/
	for (; retry < 3; retry++) {
		 err = __fw_packet_handler(ts->client);
		 if (err)
