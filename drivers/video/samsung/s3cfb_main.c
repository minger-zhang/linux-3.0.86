...
static ssize_t disp_orientation_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct s3cfb_global *fbdev = fbfimd->fbdev[0];

	return snprintf(buf, 16, "%d",
			(fbdev->lcd->width > fbdev->lcd->height) ? 0 : 90);
}
static DEVICE_ATTR(orient, 0444, disp_orientation_show, NULL);
...
static ssize_t vpost_event_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct s3cfb_global *fbdev = fbfimd->fbdev[0];

	return snprintf(buf, PAGE_SIZE, "VPOST=%llu",
			(fbdev ?  ktime_to_ns(fbdev->vsync_info.timestamp) : 0));
}

static DEVICE_ATTR(vpost_event, 0444, vpost_event_show, NULL);
...
static int s3cfb_wait_for_vsync_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		wait_event_interruptible(fbdev->vsync_info.wait,
				(count != fbdev->wq_count) && fbdev->vsync_info.active);
